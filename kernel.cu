...
__global__ void addKernel(uint8_t* buffer, double* vecl, double pos0,double pos1, double pos2, double vec0, double vec1, double vec2, double addy0, double addy1, double addy2, double addz0, double addz1, double addz2, bool inside, double alpha, double bigr, double r, bool other, uint8_t* stars)
{
	int i;
	...
	if (inside)
	{
		vl = sqrt(1 - vecn2 * vecn2);
		geoang = atan(vecn2 / vl);

		rayon = pos2 / cos(geoang);

		kappa = sin(geoang) * rayon; //(-1)*kappa
		exit = sqrt(rayon * rayon - alpha * alpha);

		pos0 += (exit+kappa) * vecn0 / vl;
		pos1 += (exit+kappa) * vecn1 / vl;
		...
		vecn1 = sqrt(1 - (exit * exit) / (rayon * rayon)) * vecn1 / vl;
		vecn2 = -exit/rayon;
	...
	for (i = 0; i < 10; i++) {
		if (torcoll != 65536)
		{
			...
			vl = sqrt(1 - nvecn[2] * nvecn[2]);
			geoang = atan(nvecn[2] / vl);
			//rayon = npos[2] / cos(geoang);
			//kappa = sin(geoang) * rayon; //(-1)*kappa
			kappa = npos[2] * tan(geoang);
